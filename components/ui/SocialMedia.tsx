...
export default function SocialLinks() {
  return (
    <div className="flex items-center space-x-4 mt-6 text-sm text-slate-500 dark:text-slate-400 font-medium">
      ...
      <div className="flex space-x-3">
        {socialMediaIcons.map((social, i) => (
          <a 
            ...
            title={social.name}
            className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-800 flex items-center justify-center bg-white dark:bg-slate-900 hover:text-[#6d28d9] hover:border-[#6d28d9] transition text-sm"
          >
            {social.icon}
          </a>
        ))}
